...
The database module (`@mabulu-inc/simplicity-admin-db`) handles PostgreSQL connectivity, schema introspection, system schema bootstrapping via schema-flow, and provides the default `DatabaseProvider` implementation. It converts live PostgreSQL schemas into the `SchemaMeta` model that drives all downstream modules.
...
- `@smplcty/schema-flow` — declarative DDL, migration, DB introspection
